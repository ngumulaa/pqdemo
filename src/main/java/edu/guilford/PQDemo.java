    ...
    public static void main( String[] args ) {
       ...
       System.out.println("Is the priority queue empty? " + pq.isEmpty());
        ...
        for (int i = 0; i < 20; i++) {
            ...
            // }
            // Solution : add a number to the key to make it unique for the map 
            if (!map.containsKey(key)) { 
                key = key + i; 
            }
            // Issue : What if we wnat to add data later? 
        ...
        for (int i = 0; i < 20; i++) {
            FullName name = new FullName(); 
            String key = name.getLastName();
            // if the key is not in the map, add <key, newArrayList<FullName>()>
            // so we have some placeto store the list of objects with the same last name 
